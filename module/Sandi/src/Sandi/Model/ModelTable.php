...
class ModelTable
{
    ...
    {
		$rowset = $this->tableGateway->select(array('model_id' => $model_id));
		$row = $rowset->current();
		if (!$row) {
    ...
    public function saveModel(Model $model)
    {
        ...
        if ($model_id == 0) {
            ...
            if ($this->getModel($model_id)) {
                $this->tableGateway->update($data, array('model_id' => $model_id));
            } else {
    ...
    {
        $this->tableGateway->delete(array('model_id' => $model_id));
    }
}
