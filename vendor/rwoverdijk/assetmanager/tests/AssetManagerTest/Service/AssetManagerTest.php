...
class AssetManagerTest extends PHPUnit_Framework_TestCase {
	...
	protected function getResolver($resolveTo = __FILE__) {
		...
		$resolver->expects ( $this->once () )->method ( 'resolve' )->with ( 'asset-path' )->will ( $this->returnValue ( $asset ) );
		
		return $resolver;
	}
	public function testConstruct() {
		$resolver = $this->getMock ( 'AssetManager\Resolver\ResolverInterface' );
		$assetManager = new AssetManager ( $resolver, array (
				'herp',
				'derp' 
		) );
	...
	public function testInvalidRequest() {
		...
		$resolver = $this->getMock ( 'AssetManager\Resolver\ResolverInterface' );
		$resolver->expects ( $this->any () )->method ( 'resolve' )->with ( 'asset-path' )->will ( $this->returnValue ( $asset ) );
