...
class CollectionsResolverTest extends PHPUnit_Framework_TestCase {
	public function getResolverMock() {
		...
		$resolver->expects ( $this->once () )->method ( 'resolve' )->with ( 'bacon' )->will ( $this->returnValue ( new Asset\FileAsset ( __FILE__ ) ) );
	...
	public function testSetGetAggregateResolver() {
		...
		$aggregateResolver->expects ( $this->once () )->method ( 'resolve' )->with ( 'say' )->will ( $this->returnValue ( 'world' ) );
	...
	public function testCouldNotResolve() {
		$aggregateResolver = $this->getMock ( 'AssetManager\Resolver\ResolverInterface' );
		$aggregateResolver->expects ( $this->once () )->method ( 'resolve' )->with ( 'bacon' )->will ( $this->returnValue ( null ) );
	...
	public function testResolvesToNonAsset() {
		$aggregateResolver = $this->getMock ( 'AssetManager\Resolver\ResolverInterface' );
		$aggregateResolver->expects ( $this->once () )->method ( 'resolve' )->with ( 'bacon' )->will ( $this->returnValue ( 'invalid' ) );
