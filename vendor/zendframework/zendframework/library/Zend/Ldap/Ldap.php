	...
	public function setOptions($options) {
		...
		foreach ( $permittedOptions as $key => $val ) {
			if (array_key_exists ( $key, $options )) {
				...
				unset ( $options [$key] );
				/*
				 * Enforce typing. This eliminates issues like Zend\Config\Reader\Ini returning '1' as a string (ZF-3163).
				 */
				switch ($key) {
					case 'port' :
	...
	public function bind($username = null, $password = null) {
		...
		if (empty ( $username )) {
			...
			if (! Dn::checkDn ( $username )) {
				if ($this->getBindRequiresDn ()) {
					/*
					 * moreCreds stops an infinite loop if getUsername does not return a DN and the bind requires it
					 */
					if ($moreCreds) {
						try {
