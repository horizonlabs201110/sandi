...
class AssetFilterManager implements ServiceLocatorAwareInterface, MimeResolverAwareInterface {
	...
	public function setFilters($path, AssetInterface $asset) {
		...
		foreach ( $filters as $filter ) {
			if (! empty ( $filter ['filter'] )) {
				...
				throw new Exception\RuntimeException ( 'Invalid filter supplied. Expected Filter or Service.' );
			}
		}
	}
	...
	protected function ensureByService(AssetInterface $asset, $service) {
		if (is_string ( $service )) {
			...
			throw new Exception\RuntimeException ( 'Unexpected service provided. Expected string or callback.' );
		}
	}
	 ...
	 * @param AssetInterface $asset        	
	 * @param mixed $service
	 *        	Either an instance of FilterInterface or a classname.
	 * @throws Exception\RuntimeException
	 */
