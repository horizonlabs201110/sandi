...
class RouteNotFoundStrategy extends AbstractListenerAggregate {
	...
	protected function renderTable($data, $cols, $consoleWidth) {
		...
		/*
		 * Check if the sum of x-1 columns fit inside console window width - 10 chars. If columns do not fit inside console window, then we'll just concatenate them and output as is.
		 */
		$width = 0;
		for($x = 1; $x < $cols; $x += 1) {
			$width += $maxW [$x];
		...
		/*
		 * Use Zend\Text\Table to render the table. The last column will use the remaining space in console window (minus 1 character to prevent double wrapping at the edge of the screen).
		 */
