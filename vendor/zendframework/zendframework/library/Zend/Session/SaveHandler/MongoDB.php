...
class MongoDB implements SaveHandlerInterface {
	...
	public function write($id, $data) {
		...
		/*
		 * Note: a MongoCursorException will be thrown if a record with this ID already exists with a different session name, since the upsert query cannot insert a new document with the same ID and new session name. This should only happen if ID's are not unique or if the session name is altered mid-process.
		 */
	...
	public function gc($maxlifetime) {
		/*
		 * Note: unlike DbTableGateway, we do not use the lifetime field in each document. Doing so would require a $where query to work with the computed value (modified + lifetime) and be very inefficient.
		 */
		$result = $this->mongoCollection->remove ( array (
